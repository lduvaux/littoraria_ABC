...
@setup MSSimulation begin
    ...
    @par history        [ [:time => TBS, :type => :mig, :rates => ["m23" => [3, 2, TBS], "m32" => [2, 3, TBS] ] ],
                          [:time => TBS, :type => :mig, :rates => ["m23" => [3, 2, 0], "m32" => [2, 3, 0] ] ],
...
@level dataset begin
    ...
    T1b = min(T2, T3)                   # min(T2, T3) years
    T1a = rand() * (T1b - 0) + 0        # [0 - T1b] years
    ...
    @par history        [T1a, M23, T1a, M32, T1b, T1b, T2, T2, N5, T3, T3, N6, T4, T4, N7]
end
...
@level locus begin
    ...
    # They are respectively in positions 2 and 4 of the
    # vector history (called history_fixed in this section)
        # set new parameter names to use at @level locus
    mig_B23_p = @par(mig_B23_par)
    mig_B32_p = @par(mig_B32_par)
    history_fixed = @par(history)
    ...
    M32 = mig_B32 * mig_B32_p[3] # mig_B32_p[3] is c32
        # update in vector history_fixed
    history_fixed[2] = M23[1] # don't forget the '[1]' or there will be a type convertion issue
    history_fixed[4] = M32[1] # don't forget the '[1]' or there will be a type convertion issue

    # declare vector 'history_fixed' as vector of values for parameter history
    @par history        history_fixed
end
